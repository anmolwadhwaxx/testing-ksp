...
const WithdrawApplicationPopUp = ({
    config,
    API_URL,
    requestId,
    hidePopUp,
    ...
    const handleSubmitWithdrawRequest = async () => {
        if (remarks.length == 0) {
            return notifyError("Reason is required");
        }
        ...
        try {
            const withdrawApplicationReq = await axios.patch(`${API_URL}transfer-request/withdraw/${requestId}`, data, config);
            if (withdrawApplicationReq.status === 200) {
                reportSuccess(true);
                notifySuccess("Application Successfully Withdrawn.");
            }
        } catch (error) {
    ...
    return ( <
        ...
        div className = "alertPopUp" > {
            ...
            <
            p className = "alertPopUpRemarksHeadingTxt" > Specify The Reason For Withdrawing Your Application < /p> <
            /div>
            ...
            /div> :
                ...
                <
                p className = "alerPopUpContentDescriptionTxt" > You are one step away to withdraw your transfer request.Do you really want to withdraw ? Your seniority will be removed and a new application number will be generated
            if in
            case you reapply.Click on
            continue
            if you are willing to withdraw or click on close to go back. < /p> <
            /div> <
            /div> <
            /div> <
...
export default WithdrawApplicationPopUp